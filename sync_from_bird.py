#!/usr/bin/python3
import os
from ripe_asset_sync import (
    RipeAsSetClient, load_clients, extract_client_as_sets,
    parse_bird_protocols, filter_established_sessions,
)
...
sessions = parse_bird_protocols()
estab_asns = filter_established_sessions(sessions)

members = []
for asn, as_sets in as_sets_by_asn.items():
    if asn in estab_asns:
        for s in as_sets:
            members.append(s.split("::")[1] if "::" in s else s)
